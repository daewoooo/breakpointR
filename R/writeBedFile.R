...
#' @param maskedRegions A \code{\link{GRanges}} object
#' @param hotspots A \code{\link{GRanges}} object
#' @param breaksGraph A \code{\link{GRanges}} object
#' @author Ashley Sanders, David Porubsky, Aaron Taudt
#' @export

writeBedFile <- function(index, outputDirectory, fragments=NULL, deltaWs=NULL, breakTrack=NULL, maskedRegions=NULL, hotspots=NULL, breaksGraph=NULL, bin=NA, col="103,139,139 243,165,97") {
	...
	if (!is.null(maskedRegions)) {
		...
		write.table(head_masked, file=savefile.masked.gz, row.names=FALSE, col.names=F, quote=FALSE, append=F, sep='\t')   
		...
		write.table(bedG, file=savefile.masked.gz, row.names=FALSE, col.names=F, quote=FALSE, append=T, sep='\t')
	...
	if (!is.null(hotspots)) {
		...
		write.table(head_hotspots, file=savefile.hotspots.gz, row.names=FALSE, col.names=F, quote=FALSE, append=F, sep='\t')
		if (length(hotspots)>0) {
			bedG <- as.data.frame(hotspots)[c('chromosome','start','end')]
		} else {
			bedG <- data.frame(chromosome='chr1', start=1, end=1)
		}
		write.table(bedG, file=savefile.hotspots.gz, row.names=FALSE, col.names=F, quote=FALSE, append=T, sep='\t')
	...
	if (!is.null(breaksGraph)) {
		...
		write.table(head_breaks, file=savefile.breaksGraph.gz, row.names=FALSE, col.names=F, quote=FALSE, append=F, sep='\t')
		if (length(breaksGraph)>0) {
			bedG <- as.data.frame(breaksGraph)[c('chromosome','start','end','hits')]
		} else {
			bedG <- data.frame(chromosome='chr1', start=1, end=1)
		}
		write.table(bedG, file=savefile.breaksGraph.gz, row.names=FALSE, col.names=F, quote=FALSE, append=T, sep='\t')
