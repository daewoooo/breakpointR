...
createCompositeFile <- function(file.list, chromosomes=NULL, pairedEndReads=TRUE, pair2frgm=FALSE, min.mapq=10, keep.duplicate.reads=FALSE, WC.cutoff=0.90) {
	...
	for (bamfile in file.list) {
		...
		fragments <- suppressWarnings( bam2GRanges(bamfile, pairedEndReads=pairedEndReads, pair2frgm=pair2frgm, chromosomes=chromosomes, min.mapq=min.mapq, keep.duplicate.reads=keep.duplicate.reads) )
