...
Currently \Rpackage{breakpointR} can take as an input only aligned reads stored in BAM files. All BAM files are expected to be present in a folder specified in \Rfunction{breakpointr(..., inputfolder)}. We advise to remove reads with low mapping quality. Duplicated reads are always removed. 
...
<<eval=TRUE, warning=FALSE, message=FALSE, fig.width=9.5, fig.height=5, out.width='\\textwidth'>>==
# Plotting a single library
exampleFolder <- system.file("extdata", "example_results", package="strandseqExampleData")
